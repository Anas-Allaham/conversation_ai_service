...
class SecurityAndObservabilityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        tutor_path = path == "/metrics" or path == "/v1" or path.startswith("/v1/")
        if tutor_path and request.headers.get("content-length"):
            try:
        ...
        if tutor_path and path not in self.PUBLIC_PATHS:
