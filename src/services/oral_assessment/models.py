...
class AssessmentCreateRequest(StrictModel):
    """Create the fixed A1-B2 conversational placement used by this MVP.

    The placement type, spoken language, and CEFR range are backend policy,
    not caller choices. Older clients may still send those fixed values during
    the compatibility window, but they are intentionally absent from OpenAPI.
    """
    ...
    @classmethod
    def accept_legacy_fixed_policy_fields(cls, value: Any) -> Any:
        if not isinstance(value, Mapping):
            return value
        payload = dict(value)
        expected = {
            "assessment_type": "conversational-placement",
            "target_range": [level.value for level in LEVELS],
            "language": "en",
        }
        for name, required_value in expected.items():
            if name in payload and payload[name] != required_value:
                raise ValueError(f"{name} is fixed by the placement service")
