...
def _sync_database_url(value: str) -> str:
    """Convert the core service's async SQLAlchemy URL for psycopg when needed."""

    normalized = value.strip()
    if normalized.startswith("postgresql+asyncpg://"):
        return "postgresql://" + normalized.removeprefix("postgresql+asyncpg://")
    if normalized.startswith("postgres+asyncpg://"):
        return "postgresql://" + normalized.removeprefix("postgres+asyncpg://")
    if normalized.startswith("sqlite+aiosqlite:///"):
        return "sqlite:///" + normalized.removeprefix("sqlite+aiosqlite:///")
    return normalized
...
class Settings:
    ...
    def from_env(cls, project_root: Path | None = None) -> Settings:
        ...
        database_url = _sync_database_url(
            os.getenv("ASSESSMENT_DATABASE_URL", "").strip()
            or os.getenv("DATABASE_URL", "").strip()
            or f"sqlite:///{root / 'data' / 'assessment.db'}"
        )
        ...
        return cls(
            ...
            service_token=(
                os.getenv("ASSESSMENT_SERVICE_TOKEN", "").strip()
                or os.getenv("SERVICE_API_KEY", "").strip()
                or "dev-service-token"
            ),
    ...
    def guided_scenario_path(self) -> Path:
        return Path(__file__).resolve().parents[1] / "guided_conversation" / "content"
