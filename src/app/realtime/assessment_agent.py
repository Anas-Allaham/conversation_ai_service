...
from conversation_ai.agent.pipeline import build_tts as build_conversation_tts
from conversation_ai.agent.pipeline import build_vad
...
def require_environment() -> None:
    core_settings.require_agent_environment()


def build_tts():
    return build_conversation_tts(core_settings)
...
def assessment_dispatch_metadata(ctx: agents.JobContext) -> dict[str, Any]:
    raw = getattr(getattr(ctx, "job", None), "metadata", "")
...
def build_assessment_audio_input_options() -> room_io.AudioInputOptions:
    ...
    mode = os.getenv(
        ...
        os.getenv("AUDIO_ENHANCEMENT", "quail_l"),
    ).strip().lower()
    if mode in {"", "none", "off", "false", "disabled"}:
        logger.info("[ASSESSMENT AUDIO] enhancement disabled")
    ...
    if level_text:
        ...
        enhancer = ai_coustics.audio_enhancement(
            ...
            model_parameters=ai_coustics.ModelParameters(enhancement_level=level),
        )
        logger.info("[ASSESSMENT AUDIO] QUAIL-L enabled | level=%.2f", level)
    else:
        enhancer = ai_coustics.audio_enhancement(
            model=ai_coustics.EnhancerModel.QUAIL_L,
        )
        logger.info("[ASSESSMENT AUDIO] QUAIL-L enabled | level=plugin-default")
...
async def assessment_session(ctx: agents.JobContext) -> None:
    ...
    require_environment()
    client = AssessmentClient()
    dispatch_metadata = assessment_dispatch_metadata(ctx)
    user_id = str(dispatch_metadata.get("user_id") or ctx.room.name or f"livekit-{uuid.uuid4()}")
    dispatched_assessment_id = str(dispatch_metadata.get("assessment_id") or "").strip()
    service_available = True
    try:
        if dispatched_assessment_id:
            state = await client.get_assessment_state_async(dispatched_assessment_id)
            created = {
                "assessment_id": dispatched_assessment_id,
                "current_item": state["current_prompt"],
            }
        else:
    ...
    session = AgentSession(
        ...
        llm=inference.LLM(
            model=os.getenv("LIVEKIT_LLM_MODEL", "google/gemini-2.5-flash-lite")
        ),
        tts=build_tts(),
