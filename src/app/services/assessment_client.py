...
class AssessmentClient:
    def __init__(
        ...
        self.base_url = (
            base_url or os.getenv("ASSESSMENT_SERVICE_URL", "http://127.0.0.1:8000")
        ).rstrip("/")
        self.token = (
            token
            or os.getenv("ASSESSMENT_SERVICE_TOKEN", "")
    ...
    def get_guided_learner_result(self, session_id: str) -> dict[str, Any]:
        return self._request(
            "GET",
    ...
    async def get_guided_learner_result_async(self, session_id: str) -> dict[str, Any]:
        return await asyncio.to_thread(self.get_guided_learner_result, session_id)
