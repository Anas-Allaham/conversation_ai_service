...
Gemini/OpenAI invalid output is schema-validated and retried once; exhausted failures return 503 without changing the learner level. Pronunciation failure produces a separate failed diagnostic and never blocks or changes the conversational result. Stopping the assessment service does not affect `english-tutor` because the two LiveKit agents and runtimes are isolated.
